...
class VideoUploadService :Service(), OnResponse<UniverSelObjct> {
    var list: ArrayList<ReactionsBean> = ArrayList()
    ...
    fun upload(){
        ...
        if(CommonMethods.isValidArrayList(list) && list.size>counter){
            var mBeanReactions  =  list[counter] as ReactionsBean
            if(CommonMethods.isValidString(mBeanReactions.video)){
                ...
                val team_id = mBeanReactions.team_id.toString().toRequestBody("multipart/form-data".toMediaTypeOrNull())
                val time = mBeanReactions.time.toString().toRequestBody("multipart/form-data".toMediaTypeOrNull())
                val reaction = mBeanReactions.reaction.toString().toRequestBody("multipart/form-data".toMediaTypeOrNull())
                val half = mBeanReactions.half.toString().toRequestBody("multipart/form-data".toMediaTypeOrNull())
                val mVideo = RetrofitUtils.createFilePart("video",File(mBeanReactions.video).absolutePath,"image".toMediaTypeOrNull())
                ApiCall(applicationContext).makeActions(this, false,user_id,match_id,
                    team_id,time,reaction,half,mVideo,mBeanReactions)
            }else{
                upload()
            }
        }
                ...
                Tags.SB_CREATE_MATCH_ACTION_API->{
                    try{
                        ...
                        if (mBean.status==1 && CommonMethods.isValidString(response.msg)){
                            var  mReactionsBean = Gson().fromJson<ReactionsBean>(response.msg,ReactionsBean::class.java)
